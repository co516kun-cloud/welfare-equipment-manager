...
### 実装者に判断を委ねず、田口さんに確認したいこと

| # | 確認 | この設計書の仮置き |
|---|---|---|
| A | **在庫の数え方**: 承認待ちの発注も在庫から引くか（§3 で説明） | **引く**（在庫一覧の方式に統一） |
| B | 「メンテナンス中」→「メンテナンス済み」、「不明」→「状態不明」、「準備完了」→「配送準備完了」も揃えてよいか | 揃える（`STATUS_LABEL` が正） |
| C | 承認バナーの件数に `partial_approved`（一部承認）を含めるか | 含める（承認画面と同じ） |
| D | 写真バケットを非公開（署名付きURL）にするか公開にするか | **非公開**。整備写真に利用者宅が写る可能性があるため |
| E | 管理番号の接頭辞は商品ごとに固定でよいか（例: 車いす A は常に `WC`） | 商品に接頭辞を持たせ、無ければ既存個体から推定 |

仮置きのまま実装を始めてよい。田口さんの答えが違えば、該当節だけ差し替える。
...
| 選択肢 | 意味 | 結果 |
|---|---|---|
| **式1に揃える（推奨）** | 承認待ちも「押さえた」扱い | 誰かが申請中の1台を、別の人が発注できなくなる。却下されれば戻る。二重予約が起きない |
| 式2に揃える | 承認されるまで在庫は空いている扱い | 発注は通りやすいが、承認が両方降りたときに1台足りなくなる |

推奨は式1。「申請した時点で押さえる」の方が現場の期待に合う（在庫一覧がそう見せている）。
...
```
非公開バケット（要確認 D）。表示時に**署名付き URL（有効1時間）**を発行する。
...
- **A** 在庫: 承認待ちも引く方式でよいか（推奨: 引く）
- **B** 「メンテナンス済み」「状態不明」「配送準備完了」への統一もよいか
- **C** 承認バナーに一部承認を含めるか
- **D** 写真バケットは非公開でよいか
- **E** 管理番号の接頭辞を商品ごとに持つ形でよいか
